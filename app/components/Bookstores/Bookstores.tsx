...
export const Bookstores =  () => {
    ...
    return (
        <div className="w-[19rem] h-[41rem] bg-filters rounded-xl mt-40 shadow-primary px-6 py-5">
            <h2 className="text-xl text-black font-bold mb-5">Bibliotecas</h2>
            {
                    data?.map((bookstore: BookstoresType) =>  
                            <div className="flex flex-col mb-2 w-[16rem]">
                                <div className="flex justify-between">
                                    <h3 className="text-black font-bold text-lg w-[12.8rem]">{bookstore.name}</h3>
                                    <span className="text-primaryColor font-bold text-base">{bookstore.rating} ⭐</span>
                                </div>
                                <p className="text-sm text-black">{bookstore.formatted_address}</p>
